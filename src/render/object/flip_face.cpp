...
bool flip_face::hit(const ray &r, double t_min, double t_max,
...
bool flip_face::bounding_box(double time0, double time1,
                             aabb &output_box) const {
  return ptr->bounding_box(time0, time1, output_box);
}

vec3 flip_face::random(const vec3 &origin) const { return ptr->random(origin); }
